...
function enterApp(){$('loginScreen').hidden=true;$('appShell').hidden=false;$('drawerStore').textContent=state.storeName;$('drawerCode').textContent=`Sucursal ${state.storeCode}`;render()}
...
$('logoutButton').addEventListener('click',()=>{openDrawer(false);$('appShell').hidden=true;$('loginScreen').hidden=false});$('validateButton').addEventListener('click',openValidation);$('cancelValidation').addEventListener('click',()=>$('validationDialog').close());$('validationForm').addEventListener('submit',validateMission);
