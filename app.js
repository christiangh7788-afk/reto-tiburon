...
const SFX_PATHS={
  ...
  error:'assets/audio/error.wav',streak:'assets/audio/streak.wav',rank:'assets/audio/rank-up.wav'
};
...
function load(){
  ...
  try{
    ...
    if(!Array.isArray(state.completed)||state.completed.length!==30)state.completed=Array(30).fill(false);
...
function loadAudioPrefs(){
  ...
  audioPrefs.musicVolume=Math.max(0,Math.min(1,Number(audioPrefs.musicVolume)||0));
  audioPrefs.sfxVolume=Math.max(0,Math.min(1,Number(audioPrefs.sfxVolume)||0));
  Object.entries(SFX_PATHS).forEach(([name,path])=>{
...
function desiredMusicKey(){
  const category=MISSIONS[state.day]?.category||'';
  return category==='Reto semanal'||category==='Gran final'?'weekly':'casual';
}

function updateAudioUI(){
  const musicButton=$('musicToggle');
  const effectsButton=$('effectsToggle');
  if(musicButton){
    musicButton.classList.toggle('is-off',!audioPrefs.music);
    musicButton.setAttribute('aria-pressed',String(audioPrefs.music));
    musicButton.querySelector('b').textContent=audioPrefs.music?'Activada':'Silenciada';
  }
  if(effectsButton){
    effectsButton.classList.toggle('is-off',!audioPrefs.sfx);
    effectsButton.setAttribute('aria-pressed',String(audioPrefs.sfx));
    effectsButton.querySelector('b').textContent=audioPrefs.sfx?'Activados':'Silenciados';
  }
  if($('musicVolume')){
    $('musicVolume').value=String(audioPrefs.musicVolume);
    $('musicVolume').disabled=!audioPrefs.music;
  }
  if($('effectsVolume')){
    $('effectsVolume').value=String(audioPrefs.sfxVolume);
    $('effectsVolume').disabled=!audioPrefs.sfx;
  }
  const banner=$('audioUnlockBanner');
  if(banner&&!$('appShell').hidden){
    banner.hidden=audioUnlocked||!audioPrefs.music;
    banner.classList.toggle('playing',audioUnlocked&&audioPrefs.music);
  }
}

function syncMusicForMission(force=false){
  const player=$('backgroundMusic');
  if(!player)return;
  const key=desiredMusicKey();
  const path=AUDIO_TRACKS[key];
  const trackChanged=player.dataset.track!==key;
  ...
  player.volume=audioPrefs.musicVolume;
  if(audioUnlocked&&audioPrefs.music&&!document.hidden){
    player.play().catch(()=>{
      const banner=$('audioUnlockBanner');
...
function pauseMusic(){const player=$('backgroundMusic');if(player)player.pause()}

function playSfx(name,delay=0){
  if(!audioUnlocked||!audioPrefs.sfx||!sfxBank[name])return;
  window.setTimeout(()=>{
    try{
      const sound=sfxBank[name].cloneNode(true);
      sound.volume=audioPrefs.sfxVolume;
      sound.play().catch(()=>{});
    }catch{}
  },delay);
}
...
function setMusicEnabled(enabled){
  audioPrefs.music=enabled;
  saveAudioPrefs();
  if(enabled){unlockAudio()}else{pauseMusic()}
  updateAudioUI();
}
...
function currentRank(points){
  const all=[...COMPETITORS.map(item=>({...item})),{name:state.storeName||'Mi tienda',points,me:true}]
    .sort((a,b)=>b.points-a.points);
  return all.findIndex(item=>item.me)+1;
}

function initialState(demo){
  state.demo=demo;
  state.storeName=$('storeNameInput').value.trim()||'Tienda Demo';
  state.storeCode=$('storeCodeInput').value.trim()||'001';
  ...
  if(demo){
    ...
    state.day=0;state.coins=0;state.streak=0;state.sales=0;state.completed=Array(30).fill(false);
  }
  save();
  enterApp();
  unlockAudio();
...
function enterApp(){
  ...
  $('drawerStore').textContent=state.storeName;
  $('drawerCode').textContent=`Sucursal ${state.storeCode}`;
  render();
  updateAudioUI();
}
...
function render(){
  ...
  const next=MISSIONS[Math.min(29,state.day+1)];
  const pct=Math.min(100,Math.round(state.sales/30000*100));
  const doneCount=state.completed.filter(Boolean).length;
  $('dayStat').textContent=`${state.day+1}/30`;
  $('coinsStat').textContent=format(state.coins);
  $('streakStat').textContent=`${state.streak} ${state.streak===1?'día':'días'}`;
  $('missionKicker').textContent=`Día ${state.day+1} · ${m.category}`;
  $('missionTitle').textContent=m.title;
  $('rewardValue').textContent=`+${m.reward}`;
  $('goalValue').textContent=`Meta: ${targetText(m)}`;
  $('progressPercent').textContent=`${pct}%`;
  $('progressFill').style.width=`${pct}%`;
  $('fin').style.left=`${pct}%`;
  $('salesValue').textContent=`$${format(state.sales)}`;
  $('nextTitle').textContent=state.day===29?'Premiación mensual':next.title;
  $('nextGoal').textContent=state.day===29?'Ranking final de tiendas':`Meta: ${targetText(next)}`;
  $('nextReward').textContent=state.day===29?'🏆':next.reward;
  $('validateButton').disabled=state.completed[state.day];
  $('validateButton').innerHTML=state.completed[state.day]?'<span>✓</span> MISIÓN CUMPLIDA':'<span>✓</span> VALIDAR MISIÓN';
  $('profileStore').textContent=state.storeName;
  $('profileCode').textContent=state.storeCode;
  $('profileMissions').textContent=doneCount;
  $('profileCoins').textContent=format(state.coins);
  renderRanking();
  syncMusicForMission();
}

function renderRanking(){
  const all=[...COMPETITORS,{name:state.storeName||'Mi tienda',points:state.coins,me:true}].sort((a,b)=>b.points-a.points);
  const shown=state.showFullRanking?all:all.slice(0,3);
  $('rankingList').innerHTML='';
  shown.forEach((item,index)=>{
    const row=document.createElement('div');
    row.className='rank-row';
    if(item.me)row.style.borderColor='#ff6500';
    const medalClass=index===0?'gold':index===1?'silver':'bronze';
    row.innerHTML=`<div class="medal ${medalClass}">${index+1}</div><div class="store-icon">▣</div><div class="rank-name"></div><div class="rank-score">${format(item.points)} ◉</div>`;
    row.querySelector('.rank-name').textContent=item.name;
    $('rankingList').appendChild(row);
  });
  $('rankingToggle').textContent=state.showFullRanking?'Ver top 3':'Ver ranking completo ›';
}

function openValidation(){
  const m=MISSIONS[state.day];
  $('validationTitle').textContent=m.title;
  $('validationHelp').textContent=`Meta del día: ${targetText(m)}.`;
  $('resultUnit').textContent=m.unit;
  $('resultInput').value='';
  $('evidenceInput').value='';
...
function validateMission(event){
  ...
  if(!Number.isFinite(result)||result<0){
    $('validationHelp').textContent='Captura un resultado válido.';
    playSfx('error');
    return;
  }
  if(result<m.target){
    const missing=m.target-result;
    $('validationHelp').textContent=`Todavía faltan ${m.unit==='MXN'?'$'+format(missing):format(missing)+' '+m.unit}.`;
    playSfx('error');
    return;
  }
  if(state.completed[state.day]){
    $('validationDialog').close();
    return;
  }
  const oldRank=currentRank(state.coins);
  const bonus=Math.min(state.streak*10,100);
  const earned=m.reward+bonus;
  state.completed[state.day]=true;
  state.coins+=earned;
  state.streak+=1;
  ...
  if(newRank<oldRank)playSfx('rank',1350);
  if(state.day<29)state.day+=1;
  save();
  render();
}

function buildCalendar(){
  const wrap=$('missionCalendar');
  wrap.innerHTML='';
  MISSIONS.forEach((m,index)=>{
    const button=document.createElement('button');
    button.type='button';
    button.className='mission-day';
    if(state.completed[index])button.classList.add('done');
    if(index===state.day)button.classList.add('current');
    button.innerHTML=`<small>Día ${index+1} · ${m.category}</small><strong></strong><span>${m.reward} NP · Meta ${targetText(m)}</span>`;
    button.querySelector('strong').textContent=m.title;
...
// El primer toque desbloquea el audio en navegadores móviles.
document.addEventListener('click',event=>{
  const button=event.target.closest('button');
  if(!button||button.disabled)return;
  unlockAudio();
  if(button.id!=='confirmValidation')playSfx('click');
},{capture:true});

$('newGameButton').addEventListener('click',()=>initialState(false));
$('demoButton').addEventListener('click',()=>initialState(true));
...
$('musicVolume').addEventListener('input',event=>{
  audioPrefs.musicVolume=Number(event.target.value);
  $('backgroundMusic').volume=audioPrefs.musicVolume;
  saveAudioPrefs();
});
$('effectsVolume').addEventListener('input',event=>{
  audioPrefs.sfxVolume=Number(event.target.value);
  saveAudioPrefs();
});

$('resetButton').addEventListener('click',()=>{
  if(confirm('¿Reiniciar todo el progreso de esta sucursal?')){
    const demo=state.demo;
    state.day=demo?6:0;state.coins=demo?1250:0;state.streak=demo?6:0;state.sales=demo?18900:0;
    state.completed=demo?Array.from({length:30},(_,i)=>i<6):Array(30).fill(false);
    state.evidence={};
    save();
    openDrawer(false);
    render();
...
$('validateButton').addEventListener('click',openValidation);
$('cancelValidation').addEventListener('click',()=>$('validationDialog').close());
$('validationForm').addEventListener('submit',validateMission);
$('rankingToggle').addEventListener('click',()=>{state.showFullRanking=!state.showFullRanking;renderRanking()});
$('nextMissionButton').addEventListener('click',()=>{if(state.day<29){state.day++;render();save()}});

document.querySelectorAll('[data-category]').forEach(button=>button.addEventListener('click',()=>{
  const category=button.dataset.category;
  let index=MISSIONS.findIndex((m,i)=>i>=state.day&&m.category===category);
...
document.addEventListener('visibilitychange',()=>{
  if(document.hidden){pauseMusic()}else if(audioUnlocked&&audioPrefs.music&&!$('appShell').hidden){syncMusicForMission()}
});
